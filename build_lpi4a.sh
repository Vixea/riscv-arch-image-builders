...
install_reqpkg() {
    pacman -Syu
    pacman -S make bison bc flex multipath-tools xz-utils qemu-user-static openssl riscv64-linux-gnu-gcc -y
}
...
get_riscv_system() {
    ...
    if [ -f $build_dir/*rootfs.tar.zst ]; then
        echo "clean tar..."
        rm $build_dir/*rootfs.tar.zst
    fi
    wget $archlinux_core_rootfs_addr -O rootfs.tar.zst
    if [ ! -f $build_dir/rootfs.tar.zst ]; then
    ...
    mkdir ${rootfs_dir}
    tar -xf rootfs.tar.zst -C ${rootfs_dir}
    cp -b /etc/resolv.conf ${rootfs_dir}/etc/resolv.conf

    mount --bind /dev ${rootfs_dir}/dev
    mount -t proc /proc ${rootfs_dir}/proc
    mount -t sysfs /sys ${rootfs_dir}/sys
#    mount /var/cache ${rootfs_dir}/var/cache

    chroot ${rootfs_dir} pacman -Syu
    chroot ${rootfs_dir} pacman -S alsa-utils haveged wpa_supplicant vim net-tools base networkmanager bluez shadow inetutils
    chroot ${rootfs_dir} pacman -S wget openssh parted linux-firmware linux-firmware-whence chkconfig e2fsprogs dracut libnm
    echo archlinux-riscv > ${rootfs_dir}/etc/hostname
...
mk_img() {
    ...
    mkfs.vfat -n arch-boot ${sdbootp}
    mkfs.ext4 -L arch-root ${sdrootp}
    ...
    echo "LABEL=arch-root  / ext4    defaults,noatime 0 0" > ${build_dir}/rootfs/etc/fstab
    echo "LABEL=arch-boot  /boot vfat    defaults,noatime 0 0" >> ${build_dir}/rootfs/etc/fstab
...
comp_img() {
    ...
    xz -v sd.img
    mv sd.img.xz archlinux-Minimal-LicheePi-4A-riscv64-sd.img.xz

    sha256sum archlinux-Minimal-LicheePi-4A-riscv64-sd.img.xz >> archlinux-Minimal-LicheePi-4A-riscv64-sd.img.xz.sha256
...
archlinux_core_rootfs_addr="https://archriscv.felixc.at/images/archriscv-2023-10-09.tar.zst"
